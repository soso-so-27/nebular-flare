...
import { HouseholdCareList } from "../shared/household-care-list";
import { getCatchUpItems, CatchUpItem } from "@/lib/utils-catchup";
import { CatchUpStack } from "../shared/catch-up-stack";
...
const careTypeConfig: Record<string, { icon: React.ReactNode; label: string; color: string }> = {
    breakfast: { icon: <Sun className="h-4 w-4" />, label: "朝ごはん", color: "text-orange-700 bg-brand-peach/20" },
    dinner: { icon: <Moon className="h-4 w-4" />, label: "夜ごはん", color: "text-purple-700 bg-brand-lavender/20" },
        ...
        <div className="relative min-h-screen">
            ...
            <AnimatePresence>
                {showSwipeMode && catchupItems.length > 0 && progressIndex < catchupItems.length && (
                    <motion.div
                        ...
                        animate={{ opacity: 1 }}
                        exit={{ opacity: 0 }}
                        className="fixed inset-0 z-[100] flex flex-col bg-gradient-to-br from-brand-sage to-emerald-700 overflow-hidden overscroll-none"
                    ...
                    >
                        ...
                        <div className="px-4 pb-6 pt-2 flex gap-3">
                            ...
                            <button
                                ...
                                className="flex-1 py-4 px-6 rounded-full bg-white text-emerald-700 font-medium text-base hover:bg-white/90 active:scale-95 transition-all"
                            >
                                完了
                            </button>
                        </div>
            ...
            {showSwipeMode && catchupItems.length > 0 && progressIndex >= catchupItems.length && (
                <motion.div
                    initial={{ opacity: 0 }}
                    animate={{ opacity: 1 }}
                    className="fixed inset-0 z-[100] flex flex-col items-center justify-center bg-gradient-to-br from-brand-sage to-emerald-700"
                >
                    <motion.div
                        initial={{ scale: 0 }}
                        animate={{ scale: 1 }}
                    ...
                    <button
                        onClick={() => setShowSwipeMode(false)}
                        className="px-8 py-3 rounded-full bg-white text-emerald-700 font-medium hover:bg-white/90 active:scale-95 transition-all"
                    >
                        閉じる
                    </button>
                </motion.div>
